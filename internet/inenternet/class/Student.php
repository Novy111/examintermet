...
class Student
{
    ...
    function addStudent($name, $roll_number, $dob, $class,$age) {
        // استعلام SQL لإدخال بيانات الطالب
        $query = "INSERT INTO tbl_student (name,roll_number,dob,class,age) VALUES (?, ?, ?, ?,?)";
        $paramType = "sissi"; // تحديد نوع المعلمات
        $paramValue = array(
            $name, // الاسم
    ...
    function editStudent($name, $roll_number, $dob, $class,$age ,$student_id) {
        // استعلام SQL لتعديل بيانات الطالب
        $query = "UPDATE tbl_student SET name = ?,roll_number = ?,dob = ?,class = ?,age = ? WHERE id = ?";
        $paramType = "sissii"; // تحديد نوع المعلمات
        $paramValue = array(
            $name, // الاسم
            $roll_number, // رقم اللف
            $dob, // تاريخ الميلاد
